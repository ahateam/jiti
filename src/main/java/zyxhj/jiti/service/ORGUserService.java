	...
	private void importORGUsersImpl(DruidPooledConnection conn, Long orgId, List<List<Object>> table) {
		for (List<Object> row : table) {
			...
			try {
				...
				createORGUser(conn, orgId, mobile, realName, idNumber, sex, familyRelations, resourceShares,
						assetShares, isOrgUser, address, shareCerNo, "", shareCerHolder, shareAmount, weight, roles,
						arrGroups, joTags, familyNumber, familyMaster);
...
//		getCountsByRoles(conn, orgId, roles);
	...
	// // 创建组织角色导入任务
	// public void createORGUserImportTask(DruidPooledConnection conn, Long orgId,
	...
	//
	// // 查询组织用户导入任务
	// public List<ORGUserImportTask> getORGUserImportTasks(DruidPooledConnection
	// conn, Long orgId, Long userId,
	// Integer count, Integer offset) throws Exception {
	// return orgUserImportTaskRepository.getORGUserImportTasks(conn, orgId, userId,
	// count, offset);
	// }
	//
	// // 查询组织用户导入任务信息
	// public ORGUserImportTask getORGUserImportTask(DruidPooledConnection conn,
	// Long importTaskId, Long orgId,
	// Long userId) throws Exception {
	// return orgUserImportTaskRepository.get(conn, EXP.INS().key("id",
	// importTaskId).andKey("org_id", orgId).andKey("user_id", userId));
	// }
	//
	// public void importORGUserRecord(DruidPooledConnection conn, Long orgId, Long
	// userId, String url, Long importTaskId)
	// throws Exception {
	// Integer sum = 0;
	// Integer err = 0;
	...
	// // 1行表头，17列，文件格式写死的
	// List<List<Object>> table = ExcelUtils.readExcelOnline(json.getString(o), 1,
	// 17, 0);
	// for (List<Object> row : table) {
	...
	// // 分组
	// temp =
	// CodecUtils.convertCommaStringList2JSONArray(orgUserImportRecord.groups);
	//
	// for (int i = 0; i < temp.size(); i++) {
	// String ts = StringUtils.trim(temp.getString(i));
	// if (ts.equals("null") || ts.equals("无")) {
	...
	// orgUserImportRecord.idNumber = StringUtils.join(orgId, "-",
	// orgUserImportRecord.familyNumber,
	// "-", IDUtils.getHexSimpleId());
	// }
	// if (StringUtils.isBlank(orgUserImportRecord.mobile)) {
	// orgUserImportRecord.mobile = StringUtils.join(orgId, "-",
	// orgUserImportRecord.familyNumber, "-",
	// IDUtils.getHexSimpleId());
	// }
	// sum++;
	...
	//
	// // 组织用户回调页面
	// public List<ORGUserImportRecord>
	// getORGUserImportRecords(DruidPooledConnection conn, Long orgId, Long
	// importTaskId,
	// Integer count, Integer offset) throws Exception {
	// return orgUserImportRecordRepository.getList(conn,EXP.INS().key("org_id",
	// orgId).andKey("task_id", importTaskId), count, offset);
	//
	// }
	//
	// private void imp(DruidPooledConnection conn, Long orgId,
	// List<ORGUserImportRecord> orgUserRec, Long importTaskId)
	// throws Exception {
	...
	//
	// try {
	// createORGUser(conn, orgId, mobile, realName, idNumber, address, shareCerNo,
	// "", shareCerHolder,
	...
	// // 修改导入数据状态为通过
	// orgUserImportRecordRepository.updateStatus(conn, orgUserImportRecord.id,
	// orgUserImportRecord.status);
	// // 导入任务 成功数+1
	// orgUserImportTaskRepository.countORGUserImportCompletionTask(conn,
	// importTaskId);
	// } catch (Exception e) {
	...
	// orgUserImportRecord.status);
	// // 导入任务 失败数+1
	// orgUserImportTaskRepository.countORGUserImportNotCompletionTask(conn,
	// importTaskId);
	// }
	// }
	// }
	...
	// orgUserImportTaskRepository.update(conn,EXP.INS().key("id", importTaskId),
	// orgUs, true);
	//
	//
	// // 把数据取出进行处理
	// ORGUserImportTask orgUserTa = orgUserImportTaskRepository.get(conn,
	// EXP.INS().key( "id", importTaskId));
	// for (int i = 0; i < (orgUserTa.sum / 100) + 1; i++) {
	// // 从导入任务数据表中取出数据
	// List<ORGUserImportRecord> orgUserRec =
	// orgUserImportRecordRepository.getList(conn,
	// EXP.INS().key("org_id", orgId).andKey("task_id",
	// importTaskId).andKey("status", ORGUserImportRecord.STATUS.UNDETECTED.v()),
	// 100, 0);
	//
	// System.out.println("");
	...
	//
	// // 获取导入失败的组织用户
	// public List<ORGUserImportRecord> getNotcompletionRecord(DruidPooledConnection
	// conn, Long orgId, Long importTaskId,
	// Integer count, Integer offset) throws Exception {
	// return orgUserImportRecordRepository.getList(conn,EXP.INS().key("org_id",
	// orgId).andKey("task_id", importTaskId).andKey("status",
	// ORGUserImportRecord.STATUS.NOTCOMPLETION.v()), count, offset);
	// }
...
//							shareCerHolder, shareAmount, weight, roles, groups, tags, familyNumber, familyMaster);
					...
					createORGUser(conn, or, mobile, realName, idNumber, sex, familyRelations, resourceShares,
							assetShares, isOrgUser, address, shareCerNo, shareCerImg, shareCerHolder, shareAmount,
...
//						shareAmount, weight, roles, groups, tags, familyNumber, familyMaster);
				...
				createORGUser(conn, or, mobile, realName, idNumber, sex, familyRelations, resourceShares, assetShares,
						isOrgUser, address, shareCerNo, shareCerImg, shareCerHolder, shareAmount, weight, roles, groups,
						tags, familyNumber, familyMaster);
	...
	public User editUserMobile(DruidPooledConnection conn, Long userId, String mobile, String password)
...
//				}
	...
	public int eidtUserPassword(DruidPooledConnection conn, Long userId, String oldPassword, String newPassword)
			throws Exception {
